...
$this->title = 'Create Alteration Endorsement - ' . Yii::$app->name;
?>

<div class="alteration-endorsement-create">
    <div class="row mb-4">
        <div class="col-md-6">
            <h2 class="p-0 m-0">Create Alteration Endorsement</h2>
        </div>
        <div class="col-md-6 my-auto">
            <?= Html::beginForm(['alteration-endorsement/create'], 'post', ['id' => 'alteration-endorsement-form', 'class' => 'form-inline']) ?>
    ...
    ?>
        <?= Html::beginForm(['alteration-endorsement/create'], 'post', ['id' => 'alteration-endorsement-member-form']) ?>
        ...
        $.ajax({
            ...
            success: function(data){         
                if (data.member_no != null) {
                    ...
                    html += '<td><input type="text" class="form-control dtpckr" name="start_dates[]" value="' + data.start_date + '"></td>';
                    html += '<td><input type="text" class="form-control dtpckr" name="end_dates[]" value="' + data.end_date + '"></td>';
                    html += '<td>' + data.sum_insured + '</td>';
                    html += '<td><input type="number" class="form-control" name="sum_insureds[]" value="' + data.sum_insured_raw + '"></td>';
            ...
            success: function(data){
                ...
                for (var i = 0; i < len; i++) {
                    if (data[i].member_no != null) {
                        ...
                        html += '<td><input type="text" class="form-control dtpckr" name="start_dates[]" value="' + data[i].start_date + '"></td>';
                        html += '<td><input type="text" class="form-control dtpckr" name="end_dates[]" value="' + data[i].end_date + '"></td>';
                        html += '<td>' + data[i].sum_insured + '</td>';
                        html += '<td><input type="number" class="form-control" name="sum_insureds[]" value="' + data[i].sum_insured_raw + '"></td>';
...
// Date Picker
$(document).on('click', '.dtpckr', function () {
    $(this).datepicker({
        format: "yyyy-mm-dd",
        autoclose: true,
        todayHighlight: true
