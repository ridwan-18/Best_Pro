        ...
        if(Yii::$app->request->post()){
            ...
            if($modelImport->fileImport && $modelImport->validate()){
                ...
                while(!empty($sheetData[$baseRow]['A'])){
					...
					->select(['policy_no','start_date','now() as tanggal_sekarang','TIMESTAMPDIFF(MONTH, start_date, NOW()) + 1 AS selisih_bulan','term','gross_premium'])
					->from('member')
					->where(['member_no' => $member->member_no])
					->one();
					
					$sisa_masa = $get_data_member['term'] - $get_data_member['selisih_bulan'];
					...
					var_dump($total_refund_premi);
                    $baseRow++;
                }
				 

                // Yii::$app->getSession()->setFlash('success','Success');
				// $this->redirect(array('alteration-endorsement/index'));
            }
			else{
