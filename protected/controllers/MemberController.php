...
class MemberController extends Controller
{
	...
	public function actionExport($id)
	{
		...
		foreach ($members as $member) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('K' . $baseRow, $member['gross_premium'] + $member['em_premium']);
	...
	public function actionUpdateMember()
	{
		...
		if (Yii::$app->request->post('em_type') != null) {
			if (Yii::$app->request->post('em_type') == Member::EM_MANUAL) {
				...
				$productEm = ProductEm::find()
					...
					->one();
				$rateEm = $productEm->em;
				$emPremium = $model->sum_insured * $rateEm / 1000;
			}
		}
		...
		$model->nett_premium = $nettPremium + $em;
	...
	public function actionApprove($id)
	{
		...
		foreach ($members as $member) {
			...
			// edo//
			$totalEMPremium += $member->em_premium;
			//
		...
		$batch->total_discount_premium = $totalDiscountPremium;
		//edo//
		$batch->total_extra_premium = $totalExtraPremium + $totalEMPremium;
		//
		$batch->total_saving_premium = $totalSavingPremium;
		$batch->total_nett_premium = $totalNettPremium;
		...
		$billing->due_date = Billing::getDueDate($tc->grace_period, $tc->grace_type);
		...
		$billing->type_invoice = 'Akseptasi';
