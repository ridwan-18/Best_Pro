...
class CreateMemberController extends Controller
{
	...
	public function actionIndex()
	{
		...
		$totalModel = BatchByPeserta::countAllDataproduksi($paramsGetAllProduksi);

		$pagination = new Pagination([
			'totalCount' => $totalModel,
			'pageSize' => BatchByPeserta::PAGE_SIZE,
			'pageSizeParam' => false,
		]);

		$paramsGetAllProduksi = array_merge($paramsGetAllProduksi, [
	...
	public function actionCreate()
    {
		...
		$policyNo = $model->policy_no;
		$id_loan = $model->id_loan;
		$name = $personal->name;
		$birth_date = $personal->birth_date;
		$start_date = $model->start_date;
		$end_date = $model->end_date;
		$sum_insured = $model->sum_insured;
		...
		$age = Member::getAge($quotation->age_calculate, $birth_date, $start_date);
		$term = Member::getTerm($quotation->rate_type, $start_date, $end_date);	
		
		$quotationUwLimit = QuotationUwLimit::find()
					->where(['quotation_id' => $policy->quotation_id])
					->andWhere(['<=', 'min_age', $age])
					->andWhere(['>=', 'max_age', $age])
					->andWhere(['<=', 'min_si', $sum_insured])
					->andWhere(['>=', 'max_si', $sum_insured])
					->one();
        ...
        $termYear = ($quotation->rate_type == \app\models\RateType::RATE_ROUND_UP) ? ceil($term / 12) : floor($term / 12);

        if ($quotationProduct->rate_type == \app\models\ProductRateType::AGE_TERM) {
            ...
            $quotationRate = QuotationRate::findOne([
                'quotation_id' => $policy->quotation_id,
                'term' => ($quotationProduct->period_type == \app\models\PeriodType::ANNUALLY) ? $termYear : $term
            ]);
        }
		...
		$batch = Batch::find()->where(['policy_no' => $policy->policy_no])->orderBy(['id' => SORT_DESC])->one();
		if ($batch != null) {
        ...
        $totalPremium = $sum_insured * $quotationRate->rate / 1000;
		
		$personal->personal_no = Personal::generatePersonalNo($name, $birth_date);
		$personal->name = $name;
		$personal->birth_date = $birth_date;
		$personal->id_card_no = $id_card_no;
		...
		$model->personal_no = Personal::generatePersonalNo($name, $birth_date);;
		$model->batch_no = str_pad($batchNo, 6, '0', STR_PAD_LEFT);
		$model->age = $age;
		$model->start_date = $start_date;
		$model->end_date = $end_date;
		$model->term = $term;
		$model->sum_insured = $sum_insured;
		$model->total_si = $sum_insured;
		$model->total_premium = $totalPremium;
		$model->rate_premi = $quotationRate->rate;
		$model->gross_premium = $totalPremium;
		$model->basic_premium = $totalPremium;
		$model->discount_premium = $totalPremium;
		$model->nett_premium = $totalPremium;
		$model->medical_code = $quotationUwLimit->medical_code;
		$model->em_premium = 0;
		$model->member_status = $status;
		...
		$batch->total_member = 1;
		$batch->total_member_accepted = 0;
		$batch->total_member_pending = 0;
		$batch->total_up = $sum_insured;
		$batch->total_gross_premium =  $totalPremium;
		$batch->total_discount_premium =  0;
		$batch->total_extra_premium = 0;
		$batch->total_saving_premium = $totalPremium;
		$batch->total_nett_premium =  $totalPremium;
		$batch->status = $status;
		$batch->created_by = Yii::$app->user->identity->id;
        $batch->created_at = $currentDate->format('Y-m-d H:i:s');
		if (!$batch->save(false)) {
			Yii::$app->session->setFlash('error', "Error while saving Batch");
			return $this->redirect(['create']);
		}
	 ...
	 public function actionCalculatePremium()
    {
        ...
        if (!$quotation || !$quotationCommission || !$quotationProduct) {
            return ['success' => false, 'message' => 'Quotation data incomplete'];
        }
        ...
        if (!$quotationRate) return ['success' => false, 'message' => 'Rate not found'];
	...
	public function actionUpload()
	{
		...
		while (!empty($sheetData[$baseRow]['D'])) {
			...
			if ($personal->save(false)) {
				...
				// $model_member = new Member();
				$response_member = $model->callAPIPostMemberPush($token,$policy_number,$name,$dob,$tgl_mulai,$tgl_selesai,$UP,$premi,$rate);
