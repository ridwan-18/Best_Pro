 ...
 */
class ViewMemberPegadaianController extends Controller
{
	/**
	 * @inheritDoc
	 */
	...
	public function actionIndex()
	{
		...
		$params = [
			'no_polis_nd' => Yii::$app->request->get('no_polis_nd'),
			'berkas' => Yii::$app->request->get('berkas'),
			'nomor_peserta_nd' => Yii::$app->request->get('nomor_peserta_nd'),
			'nama_peserta' => Yii::$app->request->get('nama_peserta'),
			'cif' => Yii::$app->request->get('cif'),
			'sbg' => Yii::$app->request->get('sbg'),
			'status_polis' => Yii::$app->request->get('status_polis'),
		];

		$totalModel = BatchByPesertaPegadaian::countAll($params);

		$pagination = new Pagination([
			'totalCount' => $totalModel,
			'pageSize' => BatchByPesertaPegadaian::PAGE_SIZE,
			'pageSizeParam' => false,
		]);
		...
		$models = BatchByPesertaPegadaian::getAllParticipantByFilter($params);
	 ...
	 */
	public function actionView($nomor_peserta_nd)
	{
		$model = $this->findModel($nomor_peserta_nd);
	...
	public function actionUpload()
	{
		...
		while (!empty($sheetData[$baseRow]['D'])) {
			...
			if ($personal->save(false)) {
				...
				// update by edo 11-08-2023//	 
				$cekAcumulate = (new \yii\db\Query())
					->select('sum(sum_insured) as Total')
					->from('Member')
					->where(['member_status' => 'Inforce','personal_no' =>$personal->personal_no])
				...
				if($cekAcumulate != 0)
				{
					$sub_total_Acumulate = ((int)$cekAcumulate['Total'] + (int)$sumInsured);
				...
				if ($statusReason != '' || $quotationUwLimit->medical_code != 'GOA' ) {
					$status = Member::MEMBER_STATUS_PENDING;
				}
	...
	public function actionApprove($nomor_peserta_nd)
	{
		if (
			Yii::$app->user->isGuest
			|| !User::findIdentityByAccessToken(Yii::$app->user->identity->access_token)
		...
		$member = ViewMemberPegadaian::findOne(['nomor_peserta_nd' => $nomor_peserta_nd]);
		if ($member == null) {
			Yii::$app->session->setFlash('error', "member not found");
			return $this->redirect(['index']);
		}
		
		$member->status_polis = ViewMemberPegadaian::STATUS_CLAIM;
		if (!$member->save(false)) {
			Yii::$app->session->setFlash('error', "Error while saving member");
			return $this->redirect(['view', 'nomor_peserta_nd' => $nomor_peserta_nd]);
		}
	...
	public function actionCancelApprove($nomor_peserta_nd)
	{
		if (
			Yii::$app->user->isGuest
			|| !User::findIdentityByAccessToken(Yii::$app->user->identity->access_token)
		...
		$member->status_polis = ViewMemberPegadaian::STATUS_CANCEL_CLAIM;
		if (!$member->save(false)) {
			Yii::$app->session->setFlash('error', "Error while saving member");
			return $this->redirect(['view', 'nomor_peserta_nd' => $nomor_peserta_nd]);
		}
	 ...
	 * @throws NotFoundHttpException if the model cannot be found
	 */
	protected function findModel($nomor_peserta_nd)
	{
		if (($model = ViewMemberPegadaian::findOne(['nomor_peserta_nd' => $nomor_peserta_nd])) !== null) {
			return $model;
		}
